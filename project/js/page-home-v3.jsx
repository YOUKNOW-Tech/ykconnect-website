// YOUKNOW Connect, Home v3 "Night Edition" sections
// BroadcastHero (cursor spotlight), ServicesRail (scroll-snap filmstrip), NoYesList, MiniSteps, CtaNavy

function BroadcastHero() {
  const [m, setM] = React.useState({ x: 38, y: 42 });
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: 32, alignItems: 'center' }} className="nt-grid2">
          <div>
            ...
            <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap', margin: '30px 0 0' }}>
              <Btn intent="on-dark" size="lg" href="contact.html">Let's connect</Btn>
...
const RAIL_SERVICES = [
  { n: '01', tag: 'CEP',  title: 'Customer Engagement',    desc: 'Real-time, personalised messaging across email, push, SMS, in-app, and WhatsApp.', partners: ['Braze', 'OneSignal', 'InsiderOne'], href: 'services-cep.html', img: 'assets/collages/customer-engagement-strategy.png', bg: 'var(--ykc-beige-300)' },
  { n: '02', tag: 'CDP',  title: 'Customer Data Platform', desc: 'Every version of your customer, stitched into one profile your whole business can trust.', partners: ['Amperity'], href: 'services-cdp.html', img: 'assets/collages/team.png', bg: 'var(--ykc-blue-100)' },
  { n: '03', tag: 'PA',   title: 'Product Analytics',      desc: "The answers GA4 can't give you: funnels, retention and adoption, without waiting on an analyst.", partners: ['Amplitude'], href: 'services-pa.html', img: 'assets/collages/reading-analytics-insights.png', bg: 'var(--ykc-beige-300)' },
  { n: '04', tag: 'BI',   title: 'Business Intelligence',  desc: 'Dashboards the whole business actually uses, not just the data team.', partners: ['DOMO'], href: 'services-bi.html', img: 'assets/collages/art-of-reporting.png', bg: 'var(--ykc-blue-100)' },
  { n: '05', tag: 'ATTR', title: 'Mobile Attribution',     desc: 'The truth about where your app users come from, plus deep links that actually land.', partners: ['AppsFlyer', 'Branch'], href: 'services-attribution.html', img: 'assets/collages/magnifying-glass.png', bg: 'var(--ykc-beige-300)' },
];
      ...
      <div className="wrap">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 56, alignItems: 'start' }} className="ny-grid">
          <div>
            ...
            <ul style={{ margin: 0, padding: 0 }}>{nos.map((t, i) => item(t, false, i))}</ul>
          </div>
          <div>
            <div style={{ marginBottom: 14 }}>
              <Sticker bg="var(--ykc-blue-500)" color="white" rotate={2} size="sm">ALWAYS INCLUDED</Sticker>
            </div>
            <ul style={{ margin: 0, padding: 0 }}>{yeses.map((t, i) => item(t, true, i))}</ul>
          </div>
        </div>
...
Object.assign(window, { BroadcastHero, ServicesRail, NoYesList, MiniSteps, CtaNavy });
